...
def refine_boundary(mesh, times):
    for i in range(times):
          g = (max(x_1,y_1)-max(x_0,y_0))*0.03
          cell_domains = MeshFunction("bool", mesh, 2)
          cell_domains.set_all(False)
          for cell in cells(mesh):
              x = cell.midpoint()
              if  x[0] < x_0+g  or x[0] > x_1-g or x[1] > y_1-g: 
...
class Lid(SubDomain):
      def inside(self, x, on_boundary):
          return True if near(x[1],y_1) and on_boundary  else False   
...
p0=Function(Qp)       # Pressure Field t=t^n
p1=Function(Qp)       # Pressure Field t=t^n+1
rho0=Function(Q)
rho1=Function(Q)
...
def Dincomp (w):
    ...
    return 0.5*(grad(w) + tgrad(w))
def Dcomp (w):
    """ Returns the  rate of  strain  tensor """
    return 0.5*(grad(w) + tgrad(w)-(2.0/3.)*div(w)*Identity(len(u)))
...
ulidreg=Expression(('8*(1.0+tanh(8*t-4.0))*(x[0]*(1.-x[0]))*(x[0]*(1.-x[0]))','0'), degree=2, t=0.0, L=L, T_0=T_0, T_h=T_h) # Lid Speed 
...
bc_pressure_at_point = DirichletBC(Qp, Constant(0), origin, 'pointwise')



# Dirichlet Boundary Conditions  (LID DRIVEN CAVITY)
noslip  = DirichletBC(V, Constant((0.0, 0.0)), no_slip)  # No Slip boundary conditions on the left wall
drive1  =  DirichletBC(V, ulidreg, lid)  # No Slip boundary conditions on the upper wall
